...
export const MeetingPreparationPage: React.FC<MeetingPreparationPageProps> = ({
  ...
  const isLocked = activeOperation !== null;
      ...
      <div className="shrink-0 border-b border-border-subtle bg-bg-secondary/80 px-8 py-5 backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            <button type="button" onClick={() => { void leave(); }} disabled={isLocked} aria-label="返回" className="rounded-full p-2 text-text-secondary transition-colors hover:bg-bg-elevated hover:text-text-primary disabled:opacity-40">
              <ArrowLeft size={18} />
            </button>
            <div>
            ...
            <section className="grid gap-5 lg:grid-cols-[1fr_250px]">
              <div className="rounded-[24px] border border-border-subtle bg-bg-elevated p-6">
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => { void parseInput(); }} disabled={isLocked || isRecording || !record.rawInput.trim()} className="flex items-center gap-2 rounded-full bg-sky-500 px-5 py-2.5 text-[12px] font-semibold text-white disabled:opacity-40">
                  ...
                  <article key={question.id} data-testid="preparation-question" className="rounded-[24px] border border-border-subtle bg-bg-elevated p-5">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <textarea aria-label={`预测问题 ${index + 1}`} disabled={isLocked} value={question.question} onChange={(event) => updateRecord((current) => ({ ...current, questions: current.questions.map((item) => item.id === question.id ? { ...item, question: event.target.value } : item) }))} className="min-h-12 w-full resize-none bg-transparent text-[14px] font-medium leading-5 outline-none" />
                        <div className="mt-2 flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] ${question.evidenceStatus === 'sufficient' ? 'bg-emerald-500/12 text-emerald-400' : question.evidenceStatus === 'missing' ? 'bg-red-500/12 text-red-400' : question.evidenceStatus === null ? 'bg-amber-500/12 text-amber-400' : 'bg-sky-500/12 text-sky-400'}`}>{question.evidence.checkError ? '检查失败' : statusLabel[question.evidenceStatus || ''] || '待检查'}</span>{question.rationale.map((item) => <span key={item} className="text-[10px] text-text-tertiary">{item}</span>)}</div>
                      </div>
                      <button type="button" disabled={isLocked} aria-label="删除问题" onClick={() => updateRecord((current) => ({ ...current, questions: current.questions.filter((item) => item.id !== question.id).map((item, sortOrder) => ({ ...item, sortOrder })) }))} className="rounded-lg p-1.5 text-text-tertiary hover:bg-red-500/10 hover:text-red-400"><Trash2 size={14} /></button>
                    </div>
                    ...
                    <div className="mt-4 flex items-center justify-end gap-2"><button type="button" onClick={() => onOpenSettings('knowledge')} className="rounded-full border border-border-subtle px-3 py-1.5 text-[10px] text-text-secondary">补充资料</button><button type="button" disabled={isLocked || !question.question.trim()} onClick={() => { void recheckQuestion(question.id); }} className="flex items-center gap-1.5 rounded-full border border-sky-400/30 bg-sky-500/10 px-3 py-1.5 text-[10px] text-sky-300 disabled:opacity-40">{activeOperation === 'recheck' ? <Loader2 size={11} className="animate-spin" /> : <RefreshCw size={11} />}重新检查</button></div>
                  </article>
                ))}
              </div>

              <div className="sticky bottom-0 flex items-center justify-between gap-4 border-t border-border-subtle bg-bg-primary/90 py-4 backdrop-blur-xl"><span className="text-[11px] text-text-tertiary">准备结果不会注入会中回答；只应用已确认模式。</span><button type="button" disabled={isLocked} onClick={() => { void startPreparedMeeting(); }} className="flex items-center gap-2 rounded-full bg-emerald-500 px-6 py-3 text-[12px] font-semibold text-white shadow-lg shadow-emerald-500/20 disabled:opacity-40"><Check size={14} />使用推荐模式开始会议</button></div>
            </section>
          )}
        </div>
      </main>
