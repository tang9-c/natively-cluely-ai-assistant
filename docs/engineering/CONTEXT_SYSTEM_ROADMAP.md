...
1. 补齐实时答案可信度闭环的验收指标和回归评测。
2. 把 `RealtimeContextOrchestrator` 从“主链路第一版”推进到“所有实时回答关键路径一致使用”。
3. 继续产品化本地 RAG、资料上传和统一知识源设置。
4. 把受控业务系统上下文从 demo/fixture 级验证推进到可配置、可诊断、可审计的只读能力。
5. 扩大短期上下文和说话人稳定性的评测覆盖。
6. 建立长期记忆 V1。
7. 建立通用只读企业知识连接器。
8. 最后再进入通用 MCP 只读适配器；可写工具仍不进入 P0。
...
```text
当前冲刺：
  1. Phase 0/2/3/4 已落地能力的验收收口。
  2. 建立上下文质量回归评测命令和指标输出。
  3. 清点所有实时回答相关 LLM 路径，标记已迁移、待迁移或豁免。

下一冲刺：
  4. 本地 RAG/资料上传端到端验收。
  5. 业务系统上下文查询状态、降级提示和超时/缓存策略。
  6. SenseVoice 错词表来源自动化和 homophone replacer 调研验证。
