...
- **CueUp API Promo Toaster redesign** — Iridescent animated border (Aurora UI, 6 s loop), breathing glow, SVG grain texture, gradient headlines, and `useReducedMotion` gating for accessibility
...
- **Profile Intelligence settings tab** — Reordered navigation so Profile Intelligence appears immediately after CueUp API
...
- **For .dmg downloads:**
  1. Open Terminal and run:
     ```bash
     xattr -cr ~/Downloads/CueUp-2.2.0-arm64.dmg
     # Or for Intel Macs:
     xattr -cr ~/Downloads/CueUp-2.2.0-x64.dmg
     ```
