# Natively - Claude Code Project Guide
...
> **CRITICAL RULE**: This project maintains an AST-based semantic knowledge graph. **ALWAYS** call `code-review-graph` MCP tools **BEFORE** resorting to standard `Grep`, `Glob`, or `Read` file tools. Standard file scanning causes severe token waste and lacks architectural awareness.

### High-Priority Decision Tree

- **Exploring and locating code**: use `semantic_search_nodes` or `query_graph`.
- **Tracing code impact and blast radius**: use impact/flow graph tools when available instead of manually tracing imports.
- **Code review and pull requests**: use graph change-review context before reading whole files.
- **Dependency and structural tracing**: use `query_graph` patterns such as `callers_of`, `callees_of`, `imports_of`, and `tests_for`.
- **System design and onboarding**: use architecture/community graph views before suggesting structural changes.

Fallback to `rg`, `Glob`, or file reads only when the graph database does not cover the target context.

### Core Tool Reference

| Tool Name | Core Purpose |
| :--- | :--- |
| `get_minimal_context` | Fast task-level graph summary and suggested next tools |
| `semantic_search_nodes` | Finds files, functions, classes, types, and tests by name or keyword |
| `query_graph` | Resolves AST relations such as callers, callees, imports, children, and tests |
| `get_community` | Inspects graph communities and domain clusters |
| `list_graph_stats` | Confirms graph coverage and freshness |
