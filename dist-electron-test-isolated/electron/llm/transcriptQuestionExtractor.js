...
exports.toCandidateFraming = toCandidateFraming;
exports.extractLatestQuestion = extractLatestQuestion;
const transcriptCleaner_1 = require("./transcriptCleaner");
 ...
 */
function toCandidateFraming(question) {
 ...
 */
function pickSalientToken(text) {
    ...
    if (camelAll && camelAll.length > 0)
        return camelAll[camelAll.length - 1];
    ...
    for (const sentence of sentences) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (!isCapitalized)
                continue;
            if (i === 0)
                continue; // sentence-initial → capitalized by grammar
            if (CAPITALIZED_STOPWORDS.has(raw.toLowerCase()))
                continue;
...
function classifyType(q) {
    ...
    if (/\b(your (full |first |last )?name|who are you|what'?s your name|what is your name)\b/.test(t))
        return 'identity';
    if (/\b(introduce yourself|tell me about yourself|describe yourself|about yourself)\b/.test(t))
        return 'identity';
    if (/\b(who (are|is) (the|this) (candidate|person|interviewee))\b/.test(t))
        return 'identity';
    // Negotiation
 ...
 */
function extractLatestQuestion(turns, windowTurns = 6) {
    const empty = {
    ...
    const ignoredTranscriptNoise = [];
    const cleaned = (0, transcriptCleaner_1.cleanTranscript)(turns);
    ...
    for (const turn of turns) {
        if (!cleanedKey.has(`${turn.timestamp}:${turn.role}`)) {
            ...
            if (trimmed)
                ignoredTranscriptNoise.push(trimmed);
        }
    }
    ...
    if (isFollowUp) {
        ...
        for (let i = priorTurns.length - 1; i >= 0; i--) {
            if (priorTurns[i].role === 'interviewer')
                continue;
    ...
    const questionType = isFollowUp ? 'follow_up' : classifyType(latestQuestion);
    ...
    if (hasMark && hasLead)
        confidence = 0.95;
    else if (hasMark || hasLead)
        confidence = 0.8;
    if (questionType !== 'general' && confidence < 0.8)
        confidence = 0.7;
    return {
...
//# sourceMappingURL=transcriptQuestionExtractor.js.map