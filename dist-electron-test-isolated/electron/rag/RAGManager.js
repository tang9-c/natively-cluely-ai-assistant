...
class RAGManager {
    ...
    _autoReindexTimer = null;
    static AUTO_REINDEX_DEFER_MS = 15_000;
    static REINDEX_LIVE_RECHECK_MS = 30_000;
    static REINDEX_MAX_LIVE_WAITS = 20; // ~10 min cap, then bail + retry next launch
    static REINDEX_DRAIN_POLL_MS = 2_000;
    static REINDEX_MAX_DRAIN_POLLS = 900; // ~30 min cap on progress polling
    scheduleAutoReindex() {
        const activeSpace = this.embeddingPipeline.getActiveSpaceKey();
        if (!activeSpace)
            return;
        if (this.vectorStore.getIncompatibleSpaceCount(activeSpace) === 0)
            return;
