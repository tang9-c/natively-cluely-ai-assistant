...
class LLMHelper {
    ...
    constructor(apiKey, useOllama = false, ollamaModel, ollamaUrl, groqApiKey, openaiApiKey, claudeApiKey, doubaoApiKey) {
        ...
        if (doubaoApiKey) {
            ...
            this.doubaoClient = new openai_1.default({ apiKey: doubaoApiKey, baseURL: DOUBAO_BASE_URL });
    ...
    setDoubaoApiKey(apiKey) {
        ...
        this.doubaoClient = new openai_1.default({ apiKey, baseURL: DOUBAO_BASE_URL });
     ...
     */
    async generateWithDoubao(userMessage, systemPrompt, imagePaths, modelId) {
        ...
        if (imagePaths && imagePaths.length > 0) {
            ...
            for (const p of imagePaths) {
                if (fs_1.default.existsSync(p)) {
                    const data = fs_1.default.readFileSync(p);
     ...
     */
    async *streamWithDoubao(userMessage, systemPrompt, imagePaths, modelId) {
        if (this.isLocalOnlyMode)
            throw new Error("Cloud providers disabled in local-only mode");
        if (!this.doubaoClient)
            throw new Error("Doubao client not initialized");
...
[REMINDER] Your entire response MUST be in ${lang} only. Never switch to English.`;
    ...
    ignoreKnowledgeMode = false, skipModeInjection = false, extraDataScopes = []) {
        ...
        // Doubao
        if (this.isDoubaoModel(this.currentModelId) && this.doubaoClient) {
            const doubaoSystem = systemPromptOverride || prompts_1.OPENAI_SYSTEM_PROMPT;
