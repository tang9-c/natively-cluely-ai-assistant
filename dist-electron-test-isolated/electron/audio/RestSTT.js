...
const PROVIDER_CONFIGS = {
    ...
    'doubao-auc': (apiKey, region, languageKey) => {
        ...
        return {
            ...
            buildRequestBody: (audioBase64, mimeType) => {
            ...
            extractTranscript: (data) => {
                ...
                if (Array.isArray(result) && result.length > 0) {
                    return result.map((item) => item?.text || item?.transcription || '').join(' ');
                }
...
class RestSTT extends events_1.EventEmitter {
    ...
    async uploadJson(wavBuffer) {
        ...
        const submitResponse = await axios_1.default.post(submitEndpoint, requestBody, {
            headers: {
                ...this.config.authHeader,
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const queryResponse = await axios_1.default.post(queryEndpoint, {}, {
